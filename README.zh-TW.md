...
會寫入 **`data/transcripts/nt201-biblical-greek.md`** 與 **`data/outlines/nt201-biblical-greek.outline.md`**（實際路徑依解析出的 slug 而定）。
...
# 多門課（各寫入 data/transcripts/<slug>.md；勿使用 --out）：
python -m bt download nt201 nt203
```

預設輸出：`data/transcripts/<course-slug>.md`（例如 `data/transcripts/nt201-biblical-greek.md`）。

工具一律將課程大綱寫入 **`data/outlines/<course-slug>.outline.md`**（與逐字稿檔案相同主檔名）。檔案開頭為課程標題 `# …`，接著每課為 `## Lesson {n}: {課名}` 與 Markdown 項目式大綱（已去除 HTML）。若存在內嵌 `__NEXT_DATA__` 則優先使用；否則課程大綱來自 JSON:API（`include=field_lessons`）。若無法取得大綱，**`download` 會以結束代碼 4 離開**，且不會再抓取課堂頁面。
...
使用 **`GEMINI_API_KEY`**。若環境變數未設定，CLI 會從**目前工作目錄**的 **`.env`** 載入（`python-dotenv`）；行為與下方 **`paragraph-outline`** 一致。請在專案目錄（或 `data/transcripts/` 與 `.env` 所在處）執行：

```bash
# 單一課（預設：data/paragraph/<model>/<slug>.lessonNN.paragraph.md）
python -m bt paragraph nt203-greek-tools-for-bible-study --lesson 3

python -m bt paragraph nt203 --lesson 3

# 全部課程（每課一次 Gemini 請求；一個合併檔：data/paragraph/<model>/<slug>.paragraph.md）
...
讀取 **`data/transcripts/<course-slug>.md`**，擷取課程內文（僅 **`--lesson`** 指定一課，或省略時處理每個 `# Lesson N:` 區段），並**不**使用課程大綱呼叫 Gemini（每課一次請求）。系統提示要求模型為逐字稿分段且不更動用字、不為段落加標題；工具再將每課標題以 **`##`** 置於開頭（來自逐字稿的 `# Lesson N:`），並以課程 **`#`** 標題與**目錄**包裝，目錄會連結文件中**每一個** `##`–`######` 標題（依層級縮排），外層版面與 `paragraph-outline` 相同。
...
請在專案目錄（或 `data/transcripts/`、`data/outlines/`、`.env` 所在處）執行。**`paragraph-outline`** 另需 **`data/outlines/`**：

```bash
# 單一課（預設：data/paragraph-outlined/<model>/<slug>.lessonNN.paragraph-outlined.md）
python -m bt paragraph-outline nt203-greek-tools-for-bible-study --lesson 3

# 全部課程（每課一次 Gemini；一檔：data/paragraph-outlined/<model>/<slug>.paragraph-outlined.md）
...
讀取 **`data/transcripts/<course-slug>.md`** 與 **`data/outlines/<course-slug>.outline.md`**，擷取每課的**逐字稿內文**與**大綱區段**（或僅 `--lesson` 指定的一課），再呼叫 Gemini。系統指示（見 `src/bt/lesson_paragraph.py`）要求模型分段、保留用字、將大綱內嵌為標題、不重複課名、大綱最淺層使用 **`###`**，且模型輸出中不使用 **`#`** / **`##`**。使用者訊息包含逐字稿與大綱文字。
...
預設路徑：有 **`--lesson`** 時為 **`data/paragraph-outlined/<model>/<slug>.lessonNN.paragraph-outlined.md`**；無則為 **`data/paragraph-outlined/<model>/<slug>.paragraph-outlined.md`**。此處 ``<model>`` 為經檔名安全處理後的 Gemini 模型 id。每課一次 Gemini 請求。每課區塊對應逐字稿的 `## Lesson N: …`（由 `# Lesson N: …` 提升），接著為大綱分段內文。
...
**`explain-zh`**（繁體中文）與 **`explain-cn`**（簡體中文）使用與 **`paragraph-outline`** 相同的逐字稿 + 大綱輸入與 **`GEMINI_API_KEY`**，但產出學習指南式說明 Markdown，位於 **`data/explain-zh/…`** 或 **`data/explain-cn/…`**（見**輸出配置**）。模型會輸出簡短大綱對照引言區塊，以及 HTML 註解（`explain-zh-h2` / `explain-cn-h2`）供工具產生雙語 **`##`** 課名；提示詞要求聖經用語優先採**改革宗／歸正神學**慣用中文。範例：`python -m bt explain-cn nt203 --lesson 1`。

## 運作方式

1. **`download`：** 抓取**課程**頁面並收集課堂連結。從內嵌 JSON 或 JSON:API 解析課程大綱；若失敗，指令在尚未下載課堂逐字稿前即停止（結束代碼 **4**）。
2. 抓取每個課堂頁面，將 **Transcription** 區段擷取為純文字。
3. 將逐字稿 Markdown 寫入 **`data/transcripts/`**，大綱寫入 **`data/outlines/`**，含**課程標題**（來自課程頁）、**目錄**，以及每課 `# Lesson {n}: {title}`。

## 輸出配置

| 類型 | 預設路徑 |
|------|----------------|
| 課程逐字稿 | `data/transcripts/<course-slug>.md` |
| 課程大綱 | `data/outlines/<course-slug>.outline.md` |
| 分段單課（Gemini，僅逐字稿，`--lesson`） | `data/paragraph/<model>/<course-slug>.lessonNN.paragraph.md` |
| 分段整課（Gemini，僅逐字稿，全部課） | `data/paragraph/<model>/<course-slug>.paragraph.md` |
| 大綱分段單課（Gemini，`--lesson`） | `data/paragraph-outlined/<model>/<course-slug>.lessonNN.paragraph-outlined.md` |
| 大綱分段整課（Gemini，全部課） | `data/paragraph-outlined/<model>/<course-slug>.paragraph-outlined.md` |
| 中文說明單課，繁體（`explain-zh`，`--lesson`） | `data/explain-zh/<model>/<course-slug>.lessonNN.zh.md` |
| 中文說明整課，繁體（`explain-zh`，全部課） | `data/explain-zh/<model>/<course-slug>.zh.md` |
| 中文說明單課，簡體（`explain-cn`，`--lesson`） | `data/explain-cn/<model>/<course-slug>.lessonNN.cn.md` |
| 中文說明整課，簡體（`explain-cn`，全部課） | `data/explain-cn/<model>/<course-slug>.cn.md` |
